    ...
    >
      <div className="max-w-7xl mx-auto px-6 h-20 grid grid-cols-2 md:grid-cols-3 items-center">
        ...
        <Link to="/" className="flex items-center gap-3">
          <img src={LogoIcon} alt="TOP Logo" className="logo-img h-8 cursor-pointer" />
          ...
          {NAV_DATA.links.map((l) => {
            ...
            return l.path === '/about' ? (
              <Link
                key={l.name}
                to={l.path}
                className={`relative py-2 hover:text-white transition-colors group ${location.pathname === l.path ? 'text-white' : ''}`}
              >
                {l.name}
                <span className={`absolute bottom-0 left-0 h-px bg-[#D4FF00] transition-all duration-300 ${location.pathname === l.path ? 'w-full' : 'w-0 group-hover:w-full'}`} />
              </Link>
            ) : (
              <a
                key={l.name}
          ...
          {NAV_DATA.links.map((l) => (
            l.path === '/about' ? (
              <Link
                ...
                className={`hover:text-[#D4FF00] transition-colors ${location.pathname === l.path ? 'text-[#D4FF00]' : ''}`}
              >
                {l.name}
              </Link>
            ) : (
              <a
                key={l.name}
                href={isHome && l.path.startsWith('/#') ? l.path.substring(1) : l.path}
                onClick={() => setMenuOpen(false)}
